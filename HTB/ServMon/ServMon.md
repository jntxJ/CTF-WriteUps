...
![servmonHTB](https://github.com/jntxJ/Writeups/blob/master/HTB/ServMon/images/servmonHTB.jpg)

Esta maquina fue muy interesante, un metodo de explotacion que a mi parecer me dejo loco (por como se hace y por el tiempo que me demore en entenderlo), tambien apesar de aveces ser super lenta y que las personas la reseteaban demasiado por estar un exploit equivocado.

Mediante el servicio FTP encontraremos archivos interesantes, que nos daran algunos usuarios (o posibles rabbit holes :P)
Explotaremos un Local File Inclusion (url) dentro del servicio NVMS 1000 (url), para asi encontrar un archivo con contraseñas y probarlas con los usuarios obtenidos
Luego con el usuario Nadine, tendremos acceso a la maquina para tener la primera flag.
...
Usaremos una **funcion** del gran (url)S4vitar, mediante el cual extraeremos los puertos y con la herramienta xclip a nuestra clipboard, evitando tener que copiar todos los puertos uno a uno.
Para con nmap hacer el escaneo de versiones y scripts

extractPorts
![extractPorts](https://github.com/jntxJ/Writeups/blob/master/HTB/ServMon/images/extractPorts.jpg)
...
![portScan](https://github.com/jntxJ/Writeups/blob/master/HTB/ServMon/images/portScan.png)
...
Encontramos un `txt`, relacionando un Directory Traversal, lo que nos permite ver archivos del sistema en el que esta corriendo la pagina, que normalmente no deberiamos ver.

Podemos relacionar lo encontrado anteriormente con FTP, ya que sabiendo que `Windows` tiene normalmente en su raiz una carpeta llamada `Users` y tenemos dos usuarios, pero realmente uno es el que nos interesa, `Nathan` y que tiene en su escritorio en archivo `Password.txt`, entonces quedaria algo asi.
...
Perfecto, tenemos credenciales, veamos si alguna es de algun usuario
...
En base al portScan(url) recordamos que habia un servicio llamado NSClient++ corriendo sobre el puerto 8443, ¿De que trata y que podemos encontrar de el en Windows? 
...
Buscando por internet, sabemos que el archivo nsclient++.ini guardaba la password del administrador :P
...
Pero resumidamente realizaremos estos pasos:
...
3. Ejecutar el script con la ayuda la API
4. Conseguir la SHELL como Administrator
...
(Buscar y tomar screen de la documentacion)
...
(SUBRAYAR CON COLORES LO NECESARIO)

Acá usamos PUT pues para "Poner", indicandole donde esta el archivo y donde queremos subirlo, en este caso en `/scripts`

Ahora, como hacemos para ejecutarlo, buscando y buscando, en la documentacion hace referencia a `/queries`, en los cuales dentro de cada uno hay unos atributos y comandos, uno de ellos tiene una instruccion de "ejecución". Veamos que *queries* hay.
...
Finalmente, poniendonos en escucha en nuestra maquina por el puerto 443, que fue el que definimos en el *.bat* obtendremos la shell.
...
Buena maquina, en su momento estaba muy lenta y ademas las personas por seguir el exploit de internet al pie de la letra pues reseteaban la maquina, lo cual hacia peorel proceso. Pero bueno, encantado y muchas gracias por leer.
